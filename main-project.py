...
persons = ["unknown", "Bahar", "Dorna", "Mohammadamin"]
has_glasses = {
    "Bahar": False,
    "Dorna": True,
    "Mohammadamin": True
...
while True:
    ...
    print("Does {} has glasses? {}".format(persons[person_id], str(img_has_glasses)))

    if not person_id == "unknown":
        if has_glasses[persons[person_id]] and not img_has_glasses:
            if not no_glasses_alert:
                # alert the person to put on glasses
                print("ALERT : put on glasses")
                no_glasses_alert = True
        else:
            if no_glasses_alert:
                # turn off the alert
                print("TURN OFF : glasses alert")
                pass
            no_glasses_alert = False
    else:
        no_glasses_alert = False
    ...
    if person_id == "unknown":
        if not unknown_person_alert:
            ...
            print("ALERT : person is unkown")
    else:
        if unknown_person_alert:
            # turn off the alert
            print("TURN OFF : unknown alert")
        unknown_person_alert = False

    # curr_distance = get_distance()
    # if curr_distance < 30:
    #     if not short_distance_alert:
    #         # alert that distance is short
    #         print("ALERT : short distance")
    # else:
    #     if short_distance_alert:
    #         # turn off the alert
    #         print("TURN OFF : turn off alert")
    #     short_distance_alert = False
